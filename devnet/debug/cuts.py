    ...
    *, 
    ...
    version='evm-development'
        ...
        async def run(cid, h, bh):
            branch = await get_branch_hashes(session, node, cid, bh, limit=limit, version=version)
        ...
        results = await asyncio.gather(*[
            run(cid, h, bh) 
            for cid, (h, bh) in cut.hashes.items() if chains is None or cid in chains
        ])
        return { key: value for key, value in results }
...
async def get_branches_for_all_nodes(
    ...
    *,
    ...
    async def run(n):
        branches = await get_branches(session, n, chains=chains, limit=limit, version=version)
        return n, branches
    jobs = [ run(n) for n in nodes ]
    results = await asyncio.gather(*jobs)
    return { key: value for key, value in results }
    ...
    return [
        {
            ...
            for value in frozenset(level.values()) if value is not None
        }
...
def get_nodes() -> frozenset[Node]:
    ...
    nodes = frozenset([f"{n['Name']}:1848" for n in results if n['Name'].endswith('consensus')])
    return nodes
