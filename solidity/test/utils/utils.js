...
async function authorizeContracts(token, tokenInfo, authorizedTokenInfos) {
  ...
  for (const tok of authorizedTokenInfos) {
    console.log(`Authorizing ${tok.chain}:${tok.address} for ${tokenInfo.chain}:${tokenInfo.address}`);
...
async function initCrossChain(sourceToken, sourceTokenInfo, targetTokenInfo, sender, receiver, amount) {
  ...
  console.log(`transfer-crosschain status: ${receipt1.status}, at block number ${receipt1.blockNumber} with hash ${receipt1.hash}`)

  // Compute origin
  let logIndex = receipt1.logs.findIndex(log => log.topics[0] == EVENT_SIG_HASH)
...
async function redeemCrossChain(targetToken, targetTokenInfo, receiver, amount, proof) {
  ...
  let response2 = await targetToken.redeemCrossChain(receiver.address, amount, proof);
